...
class Graph:
    ...
    def __init__(self, inputs, is_weighted):
        self.inputs = inputs
    ...
    def create_adjacency_list(self):
        ...
        for arc in self.arcs:
            self.adj_list.setdefault(arc[1], [])  # Crée une liste vide si la clé n'existe pas
            if arc[0] not in self.adj_list[arc[1]]:
                self.adj_list[arc[1]].append(arc[0])
            self.adj_list.setdefault(arc[0], [])  # Crée une liste vide si la clé n'existe pas
    ...
    def is_simple(self):
        for i in range(len(self.arcs)):
            ...
            for j in range(i + 1, len(self.arcs)):
                ...
                if current_arc[0] == current_compared_arc[0] and current_arc[1] == current_compared_arc[1]:
                    return False
        return True
        ...
        if self.is_related():
            ...
            for node in self.nodes:
                if len(self.adj_list[node]) % 2:
                    ...
                    pass
            if len(self.even_nodes) >= len(self.nodes) - 2:
                return True
            else:
                return False
