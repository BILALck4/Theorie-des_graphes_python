...
root = tk.Tk()
root.geometry("1000x840+50+50")  # width x height + x + y
# root.resizable(width=False, height=False)
root.title("Application pour les graphes ")
...
def display_graph():
    new_graph = Graph(inputs=inputs, is_weighted=is_weighted_variable.get())
...
def apply_algorithm():
    my_graph = Graph(inputs=inputs, is_weighted=is_weighted_variable.get())
    algorithm_name = algorithm_input_variable.get()
    kruskal_list = []

    def draw_result_kruskal():
        final_shape = Graph(inputs=kruskal_list, is_weighted=is_weighted_variable.get())
        final_shape.display_graph()

    def draw_result_prim():
        final_shape_prim = Graph(inputs=run_prime(), is_weighted=is_weighted_variable.get())
        final_shape_prim.display_graph()

    def draw_result_dijkstra():
        final_shape_dijkstra = Graph(inputs=run_dijkstra(), is_weighted=is_weighted_variable.get())
...
def graph_info_page():
    ...
    second_page_header.pack(pady=20)
    ...
    is_weighted_label = tk.Label(second_page, text="vos arrêts ont des capacités ?")
    ...
    is_oriented_label = tk.Label(second_page, text="votre graphe est orienté ?")
    ...
    def graph_edges_info_page():
        ...
        my_canvas.create_window((300, 50), window=second_frame, anchor="nw")

        third_page_header = tk.Label(second_frame, text="Entrer Les Informations de vos arret", font="Kefa 20", pady=20)
        ...
        show_inputs_button.grid(columnspan=4)
...
def first_page():
    ...
    first_page_header = tk.Label(first__page, text="Bienvenue dans GraphNavigator", font='Kefa 32', fg='#000', padx=20,
                                 pady=20)
    first_page_header.pack()

    first_page_sub_header = tk.Label(first__page, text="Explorer Vos Graphes", font='Kefa 20', fg='#000')
    first_page_sub_header.pack(pady=10)

    first_page_header.pack()
    first_page_sub_header.pack()
    create_button = tk.Button(first__page, text="+ New Graph`", bg='GREEN', command=graph_info_page)
    create_button.configure(width=10, height=2, font='Arial 12 bold', fg='white')
